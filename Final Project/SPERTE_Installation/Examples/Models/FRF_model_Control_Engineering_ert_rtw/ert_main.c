...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    FRF_model_Control_Engineering_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(FRF_model_Control_Engineering_M) == (NULL)) &&
    !rtmGetStopRequested(FRF_model_Control_Engineering_M);
  while (runModel) {
    ...
    if (rtmStepTask(FRF_model_Control_Engineering_M, 2)
        ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(FRF_model_Control_Engineering_M->extModeInfo, 3,
        &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(FRF_model_Control_Engineering_M, true);
      }

      if (rtmGetStopRequested(FRF_model_Control_Engineering_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(FRF_model_Control_Engineering_M->extModeInfo, 3,
                       &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(FRF_model_Control_Engineering_M, true);
      }
    ...
    FRF_model_Control_Engineering_step(0);

    /* Get model outputs here */
    rtExtModeCheckEndTrigger();
    pthread_mutex_unlock(&rateTaskFcnRunningMutex[0]);
    stopRequested = !((rtmGetErrorStatus(FRF_model_Control_Engineering_M) ==
...
{
  UNUSED(sig);
  rtmSetErrorStatus(FRF_model_Control_Engineering_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  FRF_model_Control_Engineering_terminate();
  rtExtModeShutdown(3);
  sem_post(&stopSem);
  return NULL;
}
...
int main(int argc, char **argv)
{
  ...
  mwRaspiInit();
  MW_launchPyserver();
  rtmSetErrorStatus(FRF_model_Control_Engineering_M, 0);
  rtExtModeParseArgs(argc, (const char_T **)argv, NULL);

  /* Initialize model */
  FRF_model_Control_Engineering_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(FRF_model_Control_Engineering_M));
  rtExtModeCheckInit(3);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(FRF_model_Control_Engineering_M->extModeInfo, 3,
      &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(FRF_model_Control_Engineering_M, true);
    }
