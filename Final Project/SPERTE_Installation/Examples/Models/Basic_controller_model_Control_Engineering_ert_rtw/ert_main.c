...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    Basic_controller_model_Control_Engineering_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Basic_controller_model_Control_Engineering_M) ==
  ...
  while (runModel) {
    ...
    if (rtmStepTask(Basic_controller_model_Control_Engineering_M, 2)
        ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded
        (Basic_controller_model_Control_Engineering_M->extModeInfo, 3,
         &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Basic_controller_model_Control_Engineering_M, true);
      }
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(Basic_controller_model_Control_Engineering_M->extModeInfo,
                       3, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Basic_controller_model_Control_Engineering_M, true);
      }
    ...
    Basic_controller_model_Control_Engineering_step(0);

    /* Get model outputs here */
    rtExtModeCheckEndTrigger();
    pthread_mutex_unlock(&rateTaskFcnRunningMutex[0]);
    stopRequested = !((rtmGetErrorStatus
                       (Basic_controller_model_Control_Engineering_M) == (NULL))
                      && !rtmGetStopRequested
                      (Basic_controller_model_Control_Engineering_M));
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  Basic_controller_model_Control_Engineering_terminate();
  rtExtModeShutdown(3);
  sem_post(&stopSem);
  return NULL;
}
...
int main(int argc, char **argv)
{
  ...
  mwRaspiInit();
  MW_launchPyserver();
  rtmSetErrorStatus(Basic_controller_model_Control_Engineering_M, 0);
  rtExtModeParseArgs(argc, (const char_T **)argv, NULL);

  /* Initialize model */
  Basic_controller_model_Control_Engineering_initialize();
...

  {
    ...
    rtExtModeWaitForStartPkt
      (Basic_controller_model_Control_Engineering_M->extModeInfo, 3, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(Basic_controller_model_Control_Engineering_M, true);
    }
  }
