...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    Feedforward_tunning_model_Control_Engineering_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Feedforward_tunning_model_Control_Engineering_M)
  ...
  while (runModel) {
    ...
    if (rtmStepTask(Feedforward_tunning_model_Control_Engineering_M, 2)
        ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded
        (Feedforward_tunning_model_Control_Engineering_M->extModeInfo, 3,
         &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Feedforward_tunning_model_Control_Engineering_M,
                            true);
      }
    ...
    Feedforward_tunning_model_Control_Engineering_step(0);
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  Feedforward_tunning_model_Control_Engineering_terminate();
  rtExtModeShutdown(3);
  sem_post(&stopSem);
  return NULL;
}
...
int main(int argc, char **argv)
{
  ...
  mwRaspiInit();
  MW_launchPyserver();
  rtmSetErrorStatus(Feedforward_tunning_model_Control_Engineering_M, 0);
  rtExtModeParseArgs(argc, (const char_T **)argv, NULL);

  /* Initialize model */
  Feedforward_tunning_model_Control_Engineering_initialize();
...

  {
    ...
    rtExtModeWaitForStartPkt
      (Feedforward_tunning_model_Control_Engineering_M->extModeInfo, 3,
       &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(Feedforward_tunning_model_Control_Engineering_M, true);
    }
  }
